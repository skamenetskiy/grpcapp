...
func create(args []string) {
	...
	if !dirEmpty {
		die("directory '%s' exists and is not empty", name)
	}
	if !dirExists {
		mkdir(name)
	}
	...
	if err := os.WriteFile(filepath.Join(name, "go.mod"), []byte(goModData), filePerm); err != nil {
		die("failed to write go.mod: %s", err)
	}
	if err := extract(name); err != nil {
		die("failed to extract application files: %s", err)
	}
}

func generate(_ []string) {
	if err := generateProto(); err != nil {
		die("failed to generate proto: %s", err)
	}
}
